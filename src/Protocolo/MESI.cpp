...
public:
    ...
    int readMESI(std::string address, PE& peLocal, PE& peExternal1, PE& peExternal2) {
        ...
        log.logCacheRead(peLocal.processor_id, peLocal.CACHE.getEntry(address).getStatus(), address, peLocal.CACHE.getEntry(address).getID(), peLocal.CACHE.getEntry(address).getData());
